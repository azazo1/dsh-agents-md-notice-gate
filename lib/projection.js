...
/**
 * Compact deletion patch. A reverse-diff against empty content looks like
 * "file now empty" (`+++ b/path`) and dumps the whole previous body; if the
 * snapshot is missing it even collapses to an empty `<diff></diff>`. `/dev/null`
 * is the git convention for "this file is gone".
 * @param path - instruction display path.
 * @returns a header-only deletion patch.
 */
export function deletionPatch(path) {
  return `===================================================================\n--- a/${path}\n+++ /dev/null\n`;
}
...
export function projectInstructionDiff(message, snapshots) {
  ...
  for (const section of sections) {
    ...
    const removed = change?.action === 'remove' || section.action === 'remove';
    if (removed) {
      patches.push(deletionPatch(section.path));
      snapshots.delete(section.path);
      continue;
    }
    ...
    snapshots.set(section.path, section.content);
  }
