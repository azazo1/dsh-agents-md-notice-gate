...
export function apply(ctx) {
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    return { kind: 'deny', reason: 'AGENTS 变化尚未确认: 在继续调用工具之前, 请先查看指定 AGENTS.md 的变化, 并按两行格式回应. 第一行输出 ' + MARKER + '\\' + ', 第二行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 注意第一行的 `\\` 为格式需求不能少. 不能只输出 marker 或泛泛说明. 然后继续你原先的工作 (如果未结束).' };
  });
  ...
  ctx.on('agent/turn-stopping', ({ agent }) => {
    ...
    agent.steer({
      ...
      content: [{
        type: 'text',
        text: 'AGENTS 变化尚未确认. 请先查看指定文件, 按两行格式回应: 第一行是 ' + MARKER + '\\' + ', 第二行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 注意第一行的 `\\` 为格式需求不能少.'
      }],
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: [
        ...
        '当你收到 <system-reminder> 中以 "Updated instructions from:" 或 "Additional instructions from:" 或 "Instructions removed:" 等开头的消息 (表示某个 AGENTS.md / CLAUDE.md 等 workspace instructions 文件发生了变化) 时, 你必须先用两行回应确认变化. 第一行必须是 ' + MARKER + '\\' + ', 第二行必须以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 注意第一行的 `\\` 为格式需求不能少.',
