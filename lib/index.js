...
export function apply(ctx) {
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    return { kind: 'deny', reason: 'AGENTS 变化尚未确认: 在继续调用工具之前, 请先查看指定 AGENTS.md 的变化, 并单独输出包含确认标记 ' + MARKER + ' 的一行说明 (不能只包含这个标记, 必须附带变化内容说明), 作为你已注意到该变化的回应.' };
  });
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: [
        ...
        '当你收到 <system-reminder> 中以 "Updated instructions from:" 或 "Additional instructions from:" 或 "Instructions removed:" 等开头的消息 (表示某个 AGENTS.md / CLAUDE.md 等 workspace instructions 文件发生了变化) 时, 你必须先用一条单独的消息输出确认标记 ' + MARKER + ', 表示你已注意到该变化.',
