...
export function apply(ctx) {
  ...
  const hasConfirmation = (text) => {
    ...
    if (lines.length < 4 || lines[0].trim() !== MARKER || lines[1].trim() !== '') return false;
    const detail = lines[2].trim();
    if (!detail.startsWith(CONFIRMATION_PREFIX) || detail.slice(CONFIRMATION_PREFIX.length).trim().length === 0) return false;
    return lines[3].trim() === '';
  };
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    ...
    return { kind: 'deny', reason: 'AGENTS 变化尚未确认: 在继续调用工具之前, 请先查看指定 AGENTS.md 的变化, 并按四行格式回应. 第一行单独输出 ' + MARKER + ', 第二行必须为空行, 第三行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 第四行必须为空行. 不能只输出 marker 或泛泛说明. 然后继续你原先的工作 (如果未结束).' };
  });
  ...
  ctx.on('agent/turn-stopping', ({ agent }) => {
    ...
    agent.steer({
      ...
      content: [{
        type: 'text',
        text: 'AGENTS 变化尚未确认. 请先查看指定文件, 按四行格式回应: 第一行是 ' + MARKER + ', 第二行必须为空行, 第三行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 第四行必须为空行.'
      }],
  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: [
        ...
        '当你收到 <system-reminder> 中以 "Updated instructions from:" 或 "Additional instructions from:" 或 "Instructions removed:" 等开头的消息 (表示某个 AGENTS.md / CLAUDE.md 等 workspace instructions 文件发生了变化) 时, 你必须先用四行回应确认变化. 第一行必须是 ' + MARKER + ', 第二行必须为空行, 第三行必须以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 第四行必须为空行.',
