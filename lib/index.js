...
const CHANGE_CONFIRMATION_NOTICE = '需要你按照规定输出 [[ACK-AGENTS]] 和相关说明以继续工作.';
...
export function apply(ctx) {
  ...
  ctx.on('agent/turn-stopping', ({ agent }) => {
    ...
    steerContinuation(agent, 'AGENTS 变化尚未确认. 请先查看指定文件, 按两行格式回应: 第一行是 ' + MARKER + '\\' + ', 第二行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 注意第一行的 `\\` 为格式需求不能少.');
  });
    ...
    ctx.effect(() => systemPrompt.section({
      ...
      text: [
        ...
        '变化说明应简要描述新增或修改的内容. 已删除的内容不需要遵守.',
         '在你输出符合上述格式的回应之前, 所有工具调用和结束输出都会被阻止. 确认之后, 请继续你当前正在执行的任务并正常调用后续工具.',
        '不要把确认回应与工具调用放在同一条消息中. 不能只输出 ' + MARKER + ', 也不能使用泛泛的确认语句.',
        '将变化纳入考虑之后, 如果原本的工作没有结束, 立刻继续工作, 确认回应不能结束本轮. 输出确认后, 必须继续你原先未完成的工作, 最终完成后再结束本轮.'
      ].join('\n')
    }));
  }
}
