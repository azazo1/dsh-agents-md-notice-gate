  ...
  if (systemPrompt !== undefined) {
    ctx.effect(() => systemPrompt.section({
      ...
      text: [
        ...
        '当你收到 AGENTS.md 这类文件发生变化的通知 (通常是一个 diff patch) 时, 你必须先用两行回应确认变化. 第一行必须是 ' + MARKER + '\\' + ', 第二行必须以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容, 注意第一行的 `\\` 为格式需求不能少.',
        ...
        '将变化纳入考虑之后, 如果原本的工作没有结束, 立刻继续工作, 不要为了专门输出回应而结束本轮工作.'
      ].join('\n')
    }));
  }
}
