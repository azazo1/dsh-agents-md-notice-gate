...
// 规范形式仍带末尾的 `\`, 但检测层面不硬性要求: 末尾 `\` 可省略,
// marker 与 `\` 之间的空白也可接受.
export const EXPECTED_FIRST = MARKER + '\\';

function isFirstLineValid(firstTrimmed) {
  if (firstTrimmed === MARKER) return true;
  const afterMarker = firstTrimmed.slice(MARKER.length);
  return firstTrimmed.startsWith(MARKER) && /^\s*\\$/.test(afterMarker);
}
...
export function inspectConfirmation(text) {
  ...
  if (!isFirstLineValid(firstTrimmed)) {
    ...
    } else if (/^\s+\\/.test(firstTrimmed.slice(MARKER.length)) && !/^\s*\\$/.test(firstTrimmed.slice(MARKER.length))) {
      issues.push('第一行 marker 与 `\\` 之后有多余内容. 正确第一行是 ' + EXPECTED_FIRST + '.');
...
export function confirmationFailureMessage(text, kind) {
  ...
  const hint = kind === 'tools'
    ? '在继续调用工具之前, 请先查看指定 AGENTS.md 的变化, 并按两行格式重新回应. 第一行输出 ' + EXPECTED_FIRST + ', 第二行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容. 不能只输出 marker 或泛泛说明. 然后继续你原先的工作 (如果未结束).'
    : '请先查看指定文件, 按两行格式重新回应: 第一行是 ' + EXPECTED_FIRST + ', 第二行以 ' + CONFIRMATION_PREFIX + ' 开头并填写具体变化内容.';
